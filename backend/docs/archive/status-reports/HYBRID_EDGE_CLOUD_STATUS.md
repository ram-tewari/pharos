...
## 🐛 Bug Fixes Applied (April 17, 2026 - 21:30)

### Bug #1: CUDA Context Overflow (FIXED ✅)

**Problem**: Summarizer/tagger tried to process 3,218 tokens when model limit is 1,024, causing device-side assert and poisoning CUDA context for all subsequent GPU operations.

**Error Log**:
```
Token indices sequence length is longer than the specified maximum sequence length 
for this model (3218 > 1024)... Assertion srcIndex < srcSelectDimSize failed.
Failed to load embedding model: CUDA error: device-side assert triggered
```

**Root Cause**: No truncation on input text before feeding to local models. The ingestion pipeline tried to summarize the entire 13,335-character FastAPI documentation in one shot.

**Solution Applied**:
- Added character-level pre-truncation (3000 chars = ~750 tokens) in `Summarizer.summarize()` and `ZeroShotTagger.generate_tags()`
- Added `truncation=True` parameter to all pipeline calls
- Prevents token overflow before it reaches GPU

**Files Changed**:
- `backend/app/shared/ai_core.py` (lines 59-77, 147-165)

**Impact**: Edge worker can now safely process large documents without CUDA crashes.

---

### Bug #2: Windows Emoji Crash (FIXED ✅)

**Problem**: SQLAlchemy logger tried to print emoji (😉) to Windows terminal with cp1252 encoding, causing UnicodeEncodeError and crashing the worker thread.
...
**Root Cause**: Windows default terminal encoding (cp1252) doesn't support Unicode emojis. The FastAPI documentation contained "from the waiting list 😉" which triggered the crash when SQLAlchemy tried to log the INSERT statement.

**Solution Applied**:
- Added `PYTHONUTF8=1` to `.env`
- Added `PYTHONIOENCODING=utf-8` to `.env`
- Forces Python to use UTF-8 for all I/O operations on Windows

**Files Changed**:
- `backend/.env` (lines 35-36)

**Impact**: Edge worker can now safely process content with Unicode characters (emojis, special symbols, international text).

---

## ✅ Current Status: READY FOR RETRY

Both bugs are fixed. The edge worker is ready to retry the FastAPI documentation ingestion:

1. ✅ **CUDA Context**: Truncation prevents overflow
2. ✅ **Unicode Handling**: UTF-8 encoding prevents emoji crashes
3. ✅ **Queue**: Task still in Redis queue (or can be re-queued)
4. ✅ **Database**: Connection pool healthy

**Next Step**: Restart the edge worker and watch it successfully complete the ingestion.
